-- Security Fix 1: Add missing RLS policies for unprotected tables

-- Enable RLS on tables that are missing it
ALTER TABLE public.sale_payment_status ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.payment_summary ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.low_stock_alerts ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.variant_details ENABLE ROW LEVEL SECURITY;

-- Add store-based RLS policies for sale_payment_status
CREATE POLICY "Users can access sale payment status for their stores"
ON public.sale_payment_status
FOR ALL
USING (user_has_store_access(store_id));

-- Add store-based RLS policies for payment_summary
CREATE POLICY "Users can access payment summary for their stores"
ON public.payment_summary
FOR ALL
USING (user_has_store_access(store_id));
...
-- Add read-only policy for variant_details (it's a view-like table)
CREATE POLICY "Users can read variant details"
ON public.variant_details
FOR SELECT
USING (true);
