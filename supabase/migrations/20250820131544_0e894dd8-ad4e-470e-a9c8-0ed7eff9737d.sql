-- Allow negative stock for materials and improve BOM material deduction logging

-- Drop existing constraint if it exists (it might not exist, that's OK)
ALTER TABLE materials DROP CONSTRAINT IF EXISTS materials_quantity_available_check;

-- Update the BOM material deduction function to allow negative stock and improve logging
...
DROP TRIGGER IF EXISTS trigger_deduct_bom_materials_for_order_item ON sales_order_items;
CREATE TRIGGER trigger_deduct_bom_materials_for_order_item
  AFTER INSERT ON sales_order_items
  FOR EACH ROW
  EXECUTE FUNCTION deduct_bom_materials_for_order_item();

-- Also update material purchase trigger to properly log movements
