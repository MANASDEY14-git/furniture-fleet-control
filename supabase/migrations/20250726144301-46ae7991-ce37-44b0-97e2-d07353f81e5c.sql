...
DROP TRIGGER IF EXISTS assign_default_store_access ON public.profiles;
...
DROP TRIGGER IF EXISTS grant_admin_access_to_stores ON public.user_roles;
...
-- Security Fix 5: Add audit trail for sensitive table access (drop existing first)
...
-- Security Fix 6: Add logging trigger for sensitive operations (drop existing first)
DROP TRIGGER IF EXISTS log_sales_orders_operations ON public.sales_orders;
DROP TRIGGER IF EXISTS log_payments_operations ON public.payments;
DROP TRIGGER IF EXISTS log_purchases_operations ON public.purchases;
