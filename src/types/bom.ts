...
export interface BOMComponentOption {
  ...
  material_id: string;
  option_name: string;
  quantity_required?: number | null; // Per-option quantity override
  is_default?: boolean; // Marks the default/display option
  price_adjustment?: number; // Price adjustment for premium options
  materials: {
    id: string;
    name: string;
    unit?: string;
...
export interface BOMComponent {
  ...
  hourly_rate?: number;
  service_cost?: number;
  service_cost_type?: 'fixed' | 'percentage';
...
export const BOMComponentOptionSchema = z.object({
  ...
  quantity_required: z.number().positive().optional().nullable(),
...
export const BOMComponentSchema = z.object({
  ...
  service_cost_type: z.enum(['fixed', 'percentage']).default('fixed').optional(),
