        ...
        <div className="space-y-1.5">
          ...
          {payment.supplier_id && (
            <div className="flex justify-between text-sm">
              ...
              <span className="text-blue-100">{supplierName}</span>
            </div>
          )}
          {payment.description && (
            <div className="flex justify-between text-sm">
              <span className="text-blue-200">Description:</span>
              <span className="text-blue-100 truncate ml-2 max-w-[150px]">{payment.description}</span>
            </div>
          )}
        </div>
    ...
    <div className="space-y-4 md:space-y-6 pb-20 md:pb-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <CreditCard className="w-6 h-6 md:w-8 md:h-8 text-cyan-400" />
          <div>
            <h1 className="text-2xl md:text-3xl font-bold glow-text">Payments</h1>
            <p className="text-sm md:text-base text-blue-300">Track payments and receipts</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 md:gap-4">
        <Card className="futuristic-card">
          <CardContent className="p-4 md:pt-6">
            <div className="text-center">
              <p className="text-xs md:text-sm text-blue-200 mb-1">Transactions</p>
        ...
        <Card className="futuristic-card">
          <CardContent className="p-4 md:pt-6">
            <div className="text-center">
              <p className="text-xs md:text-sm text-blue-200 mb-1">Receipts</p>
        ...
        <Card className="futuristic-card">
          <CardContent className="p-4 md:pt-6">
            <div className="text-center">
              <p className="text-xs md:text-sm text-blue-200 mb-1">Payments</p>
              ...
              <div className="space-y-3">
                {filteredPayments.map(payment => (
                  <MobilePaymentCard
                    key={payment.id}
                    payment={payment}
                    storeName={getStoreName(payment.store_id)}
                    supplierName={getSupplierName(payment.supplier_id)}
      ...
      {/* Mobile FAB */}
      {isMobile && <MobileFloatingActionButton onClick={() => setIsDialogOpen(true)} />}
    </div>
  );
}
