...
import { DollarSign, TrendingUp, TrendingDown, CreditCard, Receipt, Search, Eye, Calendar } from 'lucide-react';
...
export default function EnhancedPaymentsPage() {
  ...
  } = usePaymentSummary(selectedStore);
  const {
  ...
  useEffect(() => {
    ...
    const paymentsChannel = supabase.channel('enhanced-payments-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'payments'
    }, () => {
    ...
    const salesOrdersChannel = supabase.channel('enhanced-payments-sales-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'sales_orders'
    }, () => {
  ...
  const filteredPayments = useMemo(() => {
    return payments.filter(payment => {
      ...
      const matchesSearch = payment.description?.toLowerCase().includes(searchTerm.toLowerCase()) || payment.type.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const filteredOutstandingBalances = useMemo(() => {
    return salePaymentStatus.filter(sale => {
      ...
      const matchesSearch = sale.order_number?.toLowerCase().includes(searchTerm.toLowerCase()) || sale.customer_name?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const summary = useMemo(() => {
    if (paymentSummary.length === 0) return {
      totalReceipts: 0,
      totalPayments: 0,
      netBalance: 0
    };
  ...
  return <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        ...
        <div className="flex sm:block">
          <ExportButton data={filteredPayments} filename="enhanced-payments" type="payments" />
        </div>
      </div>
      ...
      <Card className="futuristic-card">
        <CardContent className="p-4 sm:pt-6">
          <div className="space-y-4">
            {isMobile && <Button variant="ghost" onClick={() => setFiltersExpanded(!filtersExpanded)} className="w-full text-cyan-400 hover:text-cyan-300 hover:bg-cyan-900/20">
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4 ${isMobile && !filtersExpanded ? 'hidden' : ''}`}>
              <div className="relative">
                ...
                <Input placeholder="Search payments..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 neon-border bg-slate-800/50 text-blue-100 placeholder-blue-400" />
              </div>
              <StoreSelector value={selectedStore} onValueChange={setSelectedStore} stores={stores} isLoading={false} placeholder="All stores" />
              <DateFilterSelector dateFilter={dateFilter} onDateFilterChange={value => {
              setDateFilter(value);
              setCurrentPage(1);
      ...
      <Card className="futuristic-card">
        ...
        // Mobile Card Layout
        <div className="space-y-4">
              {paginatedPayments.map(payment => <Card key={payment.id} className="bg-slate-800/50 border-blue-500/30">
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {payment.description && <div className="flex justify-between text-primary">
                    ...
                    <div className="mt-4 flex justify-end">
                      {isMobile ? <Drawer>
                          <DrawerTrigger asChild>
                            <Button variant="ghost" size="sm" className="text-cyan-400 hover:text-cyan-300 hover:bg-cyan-900/20" onClick={() => setSelectedPayment(payment)}>
                            ...
                            <div className="p-4">
                              {selectedPayment && <div className="space-y-4">
                                  <div className="space-y-3">
                                    ...
                                    {selectedPayment.supplier_id && <div className="flex justify-between">
                                  ...
                                  {selectedPayment.description && <div>
                            ...
                            {selectedPayment && <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                  ...
                                  <div>
                                    ...
                                    {selectedPayment.supplier_id && <p className="text-blue-200"><strong>Supplier:</strong> {getSupplierName(selectedPayment.supplier_id)}</p>}
                                  </div>
                                </div>
                                {selectedPayment.description && <div>
        ...
        <div className="overflow-x-auto">
              <Table className="data-grid">
                ...
                <TableBody>
                  {paginatedPayments.map(payment => <TableRow key={payment.id} className="border-blue-500/20">
          ...
          {filteredPayments.length > 0 && <PaginationControls currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} showInfo={true} startItem={(currentPage - 1) * itemsPerPage + 1} endItem={Math.min(currentPage * itemsPerPage, filteredPayments.length)} totalItems={filteredPayments.length} />}
        </CardContent>
      </Card>
      ...
      <Card className="futuristic-card">
        ...
        // Mobile Card Layout
        <div className="space-y-4">
              {filteredOutstandingBalances.slice(0, 10).map(sale => <Card key={sale.sale_id} className="bg-slate-800/50 border-blue-500/30">
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-3">
                        <div>
        ...
        <div className="overflow-x-auto">
              <Table className="data-grid">
                ...
                <TableBody>
                  {filteredOutstandingBalances.slice(0, 10).map(sale => <TableRow key={sale.sale_id} className="border-blue-500/20">
          ...
          {filteredOutstandingBalances.length === 0 && <div className="text-center py-8">
