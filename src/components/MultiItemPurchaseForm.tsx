...
export default function MultiItemPurchaseForm({
  trigger
}: MultiItemPurchaseFormProps) {
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="orderNumber" className="text-blue-200">Invoice Number *</Label>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.storeId} onValueChange={value => setFormData({
                  ...formData,
                  storeId: value
                })} required>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <Button type="button" onClick={addItem} className="cyber-button text-white font-semibold">
                      ...
                      {items.map(item => <TableRow key={item.id} className="border-blue-500/20">
                          <TableCell>
                            <Select value={item.itemId} onValueChange={value => updateItem(item.id, 'itemId', value)}>
                              ...
                              <SelectContent className="bg-slate-800 border-blue-500/30">
                                {filteredItems.map(availableItem => <SelectItem key={availableItem.id} value={availableItem.id} className="text-blue-100 focus:bg-blue-800/30">
                                    {availableItem.name}
                                  </SelectItem>)}
                              </SelectContent>
                            </Select>
                          ...
                          <TableCell>
                            <Input type="number" step="0.01" value={item.unitPrice || ''} onChange={e => updateItem(item.id, 'unitPrice', parseFloat(e.target.value) || 0)} className="neon-border bg-slate-800/50 text-blue-100 w-28" min="0" />
                          </TableCell>
                          <TableCell className="text-cyan-300 font-semibold">
                            ₹{item.totalPrice.toFixed(2)}
                          </TableCell>
                          <TableCell>
                            <Button type="button" onClick={() => removeItem(item.id)} disabled={items.length === 1} variant="ghost" size="sm" className="text-red-400 hover:text-red-300 hover:bg-red-900/20">
              ...
              <Button type="submit" className="w-full cyber-button text-white font-semibold" disabled={createPurchaseOrder.isPending || !formData.storeId || !formData.supplierId || !formData.orderNumber}>
