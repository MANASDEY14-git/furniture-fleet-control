...
export function BOMCostCalculator({ item }: BOMCostCalculatorProps) {
  ...
  const componentCosts = bom?.bom_components?.filter(c => c.component_type !== 'service' || (c as any).service_cost_type !== 'percentage').map(component => {
    let baseCost = 0;
    let available = 0;
    let required = 0;
    let shortage = 0;
    ...
    if (component.component_type === 'material') {
      ...
      const effectiveCost = material ? getEffectiveCost(material) : 0;
      baseCost = effectiveCost * component.quantity_required;
  ...
  const percentageServices = bom?.bom_components?.filter(c => c.component_type === 'service' && (c as any).service_cost_type === 'percentage') || [];

  const totalComponentCost = componentCosts.reduce((sum, cost) => sum + cost.totalCost, 0);
  const totalLaborCost = laborCost * quantity;
  const subtotalBeforePercentage = totalComponentCost + totalLaborCost;
  
  const percentageServiceCost = percentageServices.reduce((sum, svc) => 
    sum + (subtotalBeforePercentage * (svc.service_cost || 0) / 100), 0);
  
  const subtotal = subtotalBeforePercentage + percentageServiceCost;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="bg-slate-700/50 border-blue-500/30">
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <Label className="text-blue-200">Additional Labor Cost per Unit</Label>
                <Input
                  type="number"
                  value={laborCost}
          ...
          <Card className="bg-slate-700/50 border-blue-500/30">
            ...
            <CardContent className="space-y-4">
              ...
              {percentageServiceCost > 0 && (
                <div className="flex justify-between">
                  <span className="text-blue-200">% Services:</span>
                  <span className="text-green-300 font-mono">
                    ₹{percentageServiceCost.toFixed(2)}
                  </span>
                </div>
              )}
              ...
              <TableBody>
                {componentCosts.map((cost, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      {cost.component.component_type === 'material' && cost.material ? (
                        <Badge variant="secondary" className="text-xs">
                          {(cost.material as any).costing_method === 'exact' ? 'Exact' : 'Avg'}
                        </Badge>
                      ) : (
                    ...
                    <TableCell className="font-mono">
                      {cost.component.component_type === 'material' && cost.material && 
                        `₹${getEffectiveCost(cost.material).toFixed(2)}`}
                      {cost.component.component_type === 'labor' && (
                ...
                {/* Percentage services */}
                {percentageServices.map((svc, idx) => (
                  <TableRow key={`pct-${idx}`}>
                    <TableCell>{svc.component_name || 'Service Fee'}</TableCell>
                    ...
                    <TableCell>{svc.service_cost}%</TableCell>
                    <TableCell><span className="text-muted-foreground">N/A</span></TableCell>
                    <TableCell className="font-mono">{svc.service_cost}%</TableCell>
                    <TableCell className="font-mono">₹{(subtotalBeforePercentage * (svc.service_cost || 0) / 100).toFixed(2)}</TableCell>
