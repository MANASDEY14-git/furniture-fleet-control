  ...
  const calculateProductionPlan = () => {
    ...
    bom.bom_components.forEach(component => {
      ...
      if (component.component_type === 'material' && component.materials) {
        ...
        materialTotal += requiredQuantity * getEffectiveCost(component.materials);
        ...
        laborTotal += hours * plannedQuantity * rate;
      } else if (component.component_type === 'service') {
        const costType = (component as any).service_cost_type || 'fixed';
        if (costType === 'percentage') {
          // Will be calculated after subtotal
          percentageServiceTotal += (component.service_cost || 0);
        } else {
          fixedServiceTotal += (component.service_cost || 0) * plannedQuantity;
        }
      }
    });

    const subtotal = materialTotal + laborTotal;
    const percentageCost = subtotal * percentageServiceTotal / 100;
    const totalCost = subtotal + fixedServiceTotal + percentageCost;

    const plan: ProductionPlan = {
      quantity: plannedQuantity,
      startDate,
      estimatedDuration: totalLaborHours + (materialShortages.length > 0 ? 8 : 0),
      laborRequired: Math.ceil(totalLaborHours / 8),
      materialShortages,
      totalCost
    };
