...
export function BOMCostBreakdown({ costCalculation, className = '' }: BOMCostBreakdownProps) {
  ...
  const pieData = Object.entries(costByType)
    .filter(([, cost]) => cost > 0)
    .map(([type, cost]) => ({
      name: type,
      value: cost,
      percentage: totalEstimatedCost > 0 ? ((cost / totalEstimatedCost) * 100).toFixed(1) : '0'
    }));
                ...
                <div key={type} className="flex justify-between items-center p-2 bg-slate-700/30 rounded">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-400">
                      {totalEstimatedCost > 0 ? ((cost / totalEstimatedCost) * 100).toFixed(1) : 0}%
                    </div>
                  </div>
                </div>
              ))}
            ...
            {componentCosts.map((cost, index) => (
              <div key={index} className="flex justify-between items-center p-3 bg-slate-700/30 rounded border border-blue-500/20">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-400">
                    {totalEstimatedCost > 0 ? ((cost.totalCost / totalEstimatedCost) * 100).toFixed(1) : 0}%
                  </div>
                </div>
              </div>
            ))}
