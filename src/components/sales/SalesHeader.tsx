...
export default function SalesHeader({
  ...
  if (isMobile) {
    return <div className="space-y-4">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-foreground">{documentType === 'quote' ? 'Quotes' : 'Sales Management'}</h1>
        ...
        <div className="flex flex-col gap-3">
          ...
          <ExportButton data={filteredOrders.map(order => ({
  ...
  return <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
         <h1 className="text-3xl font-bold text-foreground">{documentType === 'quote' ? 'Quotes' : 'Sales Management'}</h1>
        <p className="text-muted-foreground">{documentType === 'quote' ? 'Manage your quotations' : 'Track sales orders with advance payments and delivery tracking'}</p>
      </div>
