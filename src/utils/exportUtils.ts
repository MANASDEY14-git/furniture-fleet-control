...
export const formatDataForExport = (data: any[], type: 'sales' | 'purchases' | 'items' | 'payments' | 'supplier-ledger' | 'stock-ledger') => {
  switch (type) {
    case 'sales':
    ...
    case 'supplier-ledger':
      return data.map(item => ({
        ...
        Supplier: item.suppliers?.name || item.supplier_name || 'Unknown',
        Store: item.stores?.name || item.store_name || 'Unknown',
        'Transaction Type': item.transaction_type,
        'Debit Amount': item.debit_amount || 0,
        'Credit Amount': item.credit_amount || 0,
        'Balance': item.runningBalance !== undefined 
          ? `₹${Math.abs(item.runningBalance).toLocaleString('en-IN')} ${item.runningBalance >= 0 ? 'Dr' : 'Cr'}`
          : '',
    ...
    case 'stock-ledger':
      return data.map(item => ({
        ...
        'Adjustment Reason': item.adjustment_reason || ''
      }));
